...
from twitter_disaster.glove_linear_net import GloveDataSet, GloveNet, GloveTestDataSet, EMBEDDING_DIMENSION
...
PRED_DATA_PATH = os.path.dirname(__file__) + "data/sample_submission_glove_linear.csv"

dataset = GloveDataSet(TRAIN_DATA_PATH)
...
model = GloveNet(epochs=6, embedding_dimension=EMBEDDING_DIMENSION, verbose=True)
model.fit(train_loader, evaluator_loader)

test_dataset = GloveTestDataSet(TEST_DATA_PATH, glove_embedding=dataset.glove_embedding)
